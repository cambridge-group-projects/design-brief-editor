...
A local mirror + editor for the GitHub wiki at
https://github.com/cambridge-group-projects/cambridge-group-projects.github.io/wiki
...
The first run also clones the *wiki's own* content into a `wiki/` folder
here — that's a separate, one-time step `start.sh` walks you through
automatically:

- It tries a plain `git clone` of the wiki first, which just works if you
...
None of this repeats on later runs — once `wiki/` is cloned, `./start.sh`
...
This picks an unused port, starts the server, syncs `wiki/` with GitHub
...
- The `wiki/` directory is a real git clone of the wiki repo. It's the source
  of truth on disk; the editor just reads and writes files there.
- The **Browse** tab (default) renders `index.md` itself as navigation,
  ...
  **+ New** button next to each of the three 2027 lists (Accepted design
  ...
  nothing about it is written into the wiki source.
- The **All pages** tab has the old flat, filterable list of every page, for
...
- **Save & Push** commits the change (`Update <page> via local wiki editor`)
  and pushes straight to GitHub — there's no draft/review step, so make sure
...
locally in `wiki/` but is *not* pushed. Resolve it by hand in that folder
(e.g. `cd wiki && git status`), then push manually.

## Requirements

- Node.js 18+ (no `npm install` needed — this has no external dependencies)
- `git`, with push access to the wiki repo (contact whoever administers the
  wiki repo's GitHub team if you don't have it)
